...
class SandboxCodeUpdaterIT {

    private Path cloneLocation;

    private SandboxCodeUpdater sandboxCodeUpdater;

    @BeforeEach
    void prepare(@TempDir Path tempDir) throws URISyntaxException {
        Path sourceCodePath = testResourcesPath("/fake-template-repo");

        cloneLocation = tempDir.resolve("northwind");
        sandboxCodeUpdater = new SandboxCodeUpdater(
    ...
    void updates_Python_example() throws Exception {
        sandboxCodeUpdater.updateCodeExamples(cloneLocation, "https://github.com/neo4j-graph-examples/northwind");
