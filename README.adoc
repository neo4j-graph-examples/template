...
The `dbms` key is an array containing information about sample dbms installation. It has four potential keys which of `targetNeo4jVersion` is required. See link:relate.project.json[relate.project.json] for reference.
...
* `plugins`: An array of plugin IDs. For example `["apoc", "graph-data-science"]`.
* `dumpFile`: Path to a neo4j dump file that will be imported after the dbms has been created.
* `scriptFile`: Path to a cypher script that will be imported after the dbms has been created.
